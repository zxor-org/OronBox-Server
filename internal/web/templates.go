...
<article class="review-card">
  ...
  <form method="post" action="/admin/review/{{.RevisionID}}" class="decision-form review-decision-form">
    <div class="review-controls">
      <fieldset class="review-tags"><legend>内容标签</legend><div class="chip-row">{{range $attributeDefinitions}}<label><input type="checkbox" name="attributes" value="{{.ID}}" {{if containsString $reviewItem.Attributes .ID}}checked{{end}}> <span>{{.NameZH}}</span></label>{{end}}</div></fieldset>
      <label class="review-grade">审核评价<select name="curation_grade"><option value="standard">普通 · 1.00</option><option value="featured">精选 · 1.50</option></select></label>
    </div>
    <label class="review-note">审核意见<textarea name="note" rows="4" placeholder="审核结论将完整显示给创作者；退回时请直接说明需要修改的内容"></textarea></label>
...
<div class="content-grid detail-grid">
  ...
  <section class="panel"><div class="section-header"><div><h2>外部绑定</h2><p>后续发布将更新这些外部资源</p></div></div><div class="binding-list">{{range .Detail.Bindings}}<article class="binding-card"><strong>{{targetLabel .Provider}}</strong>{{if .Repository}}<span class="binding-repository">{{.Repository}}</span>{{end}}<dl>{{range .Entries}}<div><dt>{{.Label}}</dt><dd><code>{{.Value}}</code></dd></div>{{end}}</dl>{{if .ExternalURL}}<a class="row-action" href="{{.ExternalURL}}" target="_blank" rel="noopener noreferrer">打开外部页面 ↗</a>{{end}}</article>{{else}}<p class="muted">没有外部绑定</p>{{end}}</div></section>

...
<section class="panel"><div class="section-header"><div><h2>资源文件与设备适配</h2><p>下载实际安装包检查；设备绑定在此处直接核对和修正。</p></div></div><div class="artifact-review-list">{{range $current.Artifacts}}{{$artifact := .}}<article class="artifact-review-card"><div class="artifact-review-head"><div><strong>{{.OriginalName}}</strong><span class="cell-note"><code>{{.PackageID}}</code> · {{.Version}} · {{.PackageFormat}} · {{.SizeBytes}} B</span><span class="cell-note"><code>{{.SHA256}}</code></span></div><a class="filled-button" href="/admin/blobs/{{.SHA256}}?download=1&amp;name={{urlquery .OriginalName}}">下载文件</a></div><details><summary>服务端分析结果</summary><pre class="diagnostic compact-diagnostic">{{prettyJSON .Analysis}}</pre></details><form method="post" action="/admin/resources/{{$review.ResourceID}}/draft/{{$current.ID}}/artifacts/{{.ID}}/devices"><input type="hidden" name="return_review" value="{{$review.ID}}"><fieldset><legend>适配设备</legend><div class="choice-grid compact-choices">{{range $.Devices}}<label class="choice-card"><input type="checkbox" name="device_ids" value="{{.ID}}" {{if containsDevice $artifact.DeviceBindings .ID}}checked{{end}} {{if ne $review.State "pending"}}disabled{{end}}><span><strong>{{.DisplayName}}</strong><small>{{.Codename}}</small></span></label>{{end}}</div></fieldset>{{if eqs $review.State "pending"}}<button class="outlined-button">保存设备绑定</button>{{end}}</form></article>{{else}}<div class="empty-state compact"><span class="material-symbols-outlined">deployed_code_alert</span><p>本次提交没有资源文件，不能完成安装包审核。</p></div>{{end}}</div></section>

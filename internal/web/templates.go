...
<tbody>{{range .Items}}<tr>
  ...
  <td><form method="post" action="/admin/users/{{.ID}}/state" class="inline-role-form"><input type="hidden" name="action" value="set_role"><select name="role"><option value="user" {{if eqs .Role "user"}}selected{{end}}>用户</option><option value="reviewer" {{if eqs .Role "reviewer"}}selected{{end}}>审核员</option><option value="admin" {{if eqs .Role "admin"}}selected{{end}}>管理员</option></select><button class="text-link" type="submit">保存</button></form></td>
...
<script>document.querySelectorAll('[data-target-form]').forEach(function(form){var select=form.querySelector('[data-target-type]');function sync(){form.querySelectorAll('[data-target]').forEach(function(field){var active=field.dataset.target===select.value;field.hidden=!active;field.querySelectorAll('input,select,textarea').forEach(function(control){control.disabled=!active;if(!active)control.value='';});});}select.addEventListener('change',sync);sync();});(function(){var input=document.getElementById('home-cover-file');var target=null;document.querySelectorAll('[data-upload-cover]').forEach(function(button){button.addEventListener('click',function(){target=button.closest('form').querySelector('[data-cover-field]');input.click();});});input.addEventListener('change',async function(){if(!input.files.length||!target)return;var body=new FormData();body.append('file',input.files[0]);target.setCustomValidity('');try{var response=await fetch('/admin/blobs',{method:'POST',body:body});if(!response.ok)throw new Error('upload');var data=await response.json();if(!data.sha256)throw new Error('upload');target.value=data.sha256;}catch(_){target.setCustomValidity('图片上传失败，请重试');target.reportValidity();}finally{input.value='';}});})();</script>
...
<input type="hidden" id="blog-edit-updated-at" name="updated_at" value="{{rfc3339 .Post.UpdatedAt}}"><script>(function(){var marker=document.getElementById('blog-edit-updated-at');var form=document.querySelector('form.blog-editor');if(marker&&form)form.appendChild(marker);})();</script>
...
(function() {
  ...
  fileInput.addEventListener('change', async function() {
    ...
    var response;
    try {
      response = await fetch('/admin/blobs', {method: 'POST', body: body});
      ...
      if (mode === 'cover') {
        ...
        var insert = '![](/api/blobs/' + data.sha256 + ')';

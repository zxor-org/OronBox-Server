...
> Version: `1.0.0`　Effective date: `2026-07-19`　Last updated: `2026-07-19`

These Rules apply to the OronBox resource catalog and to resources submitted through OronBox to BandBBS or AstroBox-Repo. The content standards apply to every OronBox resource, not only externally synchronized resources
...
The Rules apply whether a publisher uses their own account or delegates publication to `oronbox-community`. Delegation does not reduce the creator's responsibility
...
3. OronBox proportionally limits each dimension to 1500 px and converts images to WebP
...
A revision may be a draft, ready for submission, under review, changes requested, rejected, approved, published, suspended, or delisted
...
1. **Approved:** published in the OronBox catalog and selected external jobs are started
...
Automated analysis may assist review, but serious account sanctions should receive human review where practical

## Chapter 6 — Enforcement

These measures supplement the management powers in Section 6 of the OronBox Terms and Disclaimer

Measures may include warnings; required corrections; rejection; review, display, or download suspension; version or resource removal; blocked updates; stopped R2/CDN distribution; cancelled external synchronization; publishing restrictions; renewed identity or rights verification; suspended or revoked creator eligibility; forced session termination; temporary account suspension; permanent bans; and action against accounts used to evade sanctions

OronBox may cancel jobs, stop retries, withdraw delegated pull requests, archive controlled repositories, request action from BandBBS, report to AstroBox-Repo maintainers or GitHub, and review its own decisions in light of third-party action
...
Users must not evade action through replacement BandBBS or GitHub accounts, borrowed identities, proxy publishers, changed IDs or hashes, alternate official-service interfaces, or substantially equivalent conduct. IP, device, and behavioral associations are factors only; no sanction should rely solely on one IP address or device identifier

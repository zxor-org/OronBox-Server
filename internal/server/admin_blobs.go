...
func (a *App) handleAdminBlobs(w http.ResponseWriter, r *http.Request) {
	...
	a.render(w, r, "admin_blobs", map[string]any{"Title": "Blob 与副本", "Items": page.Items, "Page": page, "Query": page.Query, "Pager": web.NewPagination("/admin/storage/blobs", r.URL.Query(), page.Page, page.PerPage, page.Total), "Action": r.URL.Query().Get("action")})
}
...
func (a *App) handleAdminBlobDetail(w http.ResponseWriter, r *http.Request) {
	...
	a.render(w, r, "admin_blob_detail", map[string]any{"Title": detail.Blob.SHA256, "Detail": detail, "Action": r.URL.Query().Get("action")})
}
